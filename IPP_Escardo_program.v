  ...
  callcc (fun start : cont ipp_stream =>
    coiter _
      ...
      (fun '(depth; rest) =>
          ...
          then (S depth; tail rest)
          else callcc (fun restart => throw start
            (coiter {n : nat & stream bool}
              (fun st => st.1)
              (fun '(depth; rest) =>
                  if bool_dec (head rest) b0
                  then throw restart (S depth; tail rest)
...
Extraction "IPP_Escardo_program.ml" test1 test2 test3 test4 test5 test6 test7  testprova1 testprova2 testprova3 testprova4 test_ET test_EF test_ET1 test_EF1 test_ET2 test_EF2 test_ET3 test_EF3.

Extraction Language Scheme.
Extract Constant callcc => "call/cc".
Extract Constant throw => "(lambda (x) x)".
Extract Constant wrap => "(lambda (f) (f '(Tt)))".
Extraction NoInline test.
Extraction "IPP_Escardo_program.scm" test_ET2 test_EF2  test1 test2 test3 test4 test5 test6 test7 testprova1 testprova2 testprova3 testprova4.
