...
(** For a given stream of Boolean values, this implementation classically
    returns either the stream of all elements equal to the first element of
    the input or a stream of consecutive elements different to the first
...
Definition state0 bs b n :=
  { sp : { depth & stream bool } | spec bs sp.2 b n (Trunc None) sp.1 }.
...
Definition untruncated_spec_swap_neg {bs rest b n n' depth} (e_b : head rest = b) (H : untruncated_spec bs rest (negb b) n n' depth)
  : untruncated_spec bs (tail rest) b n' None (S depth).
Proof.
...
Definition spec_swap_neg {bs rest b n' n depth} (e_b : head rest = b) (H : spec bs rest (negb b) n n' depth)
  : spec bs (tail rest) b n' (Trunc None) (S depth).
Proof.
destruct H, H.
constructor. constructor.
...
Definition untruncated_spec_swap {bs rest b n n' depth} (e_b : head rest = b) (H : untruncated_spec bs rest (negb b) n n' depth)
...
destruct H, H.
constructor. constructor.
eapply (untruncated_spec_swap e_b).
...
Extraction "IPP_Escardo_proof.ml" test1 test2 test3 test4 test5 test6 test7 testprova1 testprova2 testprova3 testprova4 
...
Extraction "IPP_Escardo_proof.scm" test1 test2 test3 test4 test5 test6 test7.
