...
(** val corecM :
    ('a1 -> nat) -> ('a1 -> (ipp_stream, 'a1) sum) -> 'a1 -> ipp_stream **)

let rec corecM base next s =
  lazy (Build_ipp_stream ((base s), (fun _ ->
    match next s with
    | Inl s0 -> s0
    | Inr x -> corecM base next x)))

(** val corecC :
    ('a1 -> nat) -> ('a1 -> ipp_stream cont -> 'a1) -> 'a1 -> ipp_stream **)
...
let infinite_bool bs =
  ...
  callcc (fun start ->
    coiter (fun pat ->
      ...
      (fun pat ->
      let ExistT (x, switch) = pat in
      let ExistT (depth0, rest0) = x in
      (match bool_dec' (head rest0) b0 with
       | True -> ExistT ((ExistT ((S depth0), (tail rest0))), switch)
       | False ->
         callcc (fun restart ->
           throw switch
             (corecC projT1 (fun pat0 ret ->
               let ExistT (depth1, rest1) = pat0 in
               (match bool_dec' (head rest1) b0 with
                | True ->
                  throw restart (ExistT ((ExistT ((S depth1), (tail rest1))),
                    ret))
                | False -> ExistT ((S depth1), (tail rest1)))) (ExistT ((S
               depth0), (tail rest0))))))) (ExistT ((ExistT (O, (tail bs))),
      start)))
